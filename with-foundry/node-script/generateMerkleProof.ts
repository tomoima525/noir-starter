...
export const generateMerkleProof = async (path: string, countryCode: string) => {
  let fd: number = 0;
  try {
    ...
    const zeroes = genZeroHashes(poseidon, MERKLE_TREE_DEPTH);
    const str = fs.readFileSync(path, 'utf-8');
    const nodes = JSON.parse(str);
    const leafNode = nodes.filter((node: TreeNode) => node.countryCode === countryCode)[0];
    const root = nodes.filter(node => node.level === MERKLE_TREE_DEPTH && node.index === 0)[0];
    ...
    for (let level = 0; level < MERKLE_TREE_DEPTH; level += 1) {
      ...
      for (let i = levelStartIndex; i < levelEndIndex; i += 1) {
        if (i !== index) {
          ...
          if (i < length) {
            const n = nodes.filter(node => node.id === `CountryCodeTree_${level}_${i}`)[0];
